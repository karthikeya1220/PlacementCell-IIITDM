...
import { placementRules } from "./rules-data"

export default function PlacementRulesContent() {
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case "LogIn":
        return <LogIn className="h-6 w-6 text-[#4DA8DA]" />
      case "HelpCircle":
        return <HelpCircle className="h-6 w-6 text-[#4DA8DA]" />
      default:
        return <CheckCircle className="h-6 w-6 text-[#4DA8DA]" />
    }
  }
  ...
  return (
    <section id="rules" className="py-16 md:py-24 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <div className="inline-block px-3 py-1 bg-[#4DA8DA]/10 text-[#4DA8DA] rounded-full text-sm font-medium mb-4">
            Guidelines
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-[#0A2463] mb-4">Placement Rules</h2>
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            These rules are designed to ensure a fair and transparent placement process for all students
          </p>
        </div>
